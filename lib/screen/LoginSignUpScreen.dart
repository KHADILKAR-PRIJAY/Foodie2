...
class _LoginSignUpScreenState extends State<LoginSignUpScreen> {
                              ...
                              builder: (context, projectSnap) {
                                ...
                                if (projectSnap.connectionState ==
                                  ...
                                  if (projectSnap.data != null) {
                                    result =
                                        projectSnap.data.data[0].nearbyKitchen;
                                    if (result != null) {
                                      print(result.length);
  ...
  void socailLogin(FirebaseUser user) async {
    ...
    try {
      ...
      SocailLogin bean = await ApiProvider().socailLogin(data);
      print(bean.data);
      _progressDialog.dismiss();
      if (bean.status == true) {
        PrefManager.putBool(AppConstant.session, true);
  ...
  Future<GetAddressList> getAddress(BuildContext context) async {
    _progressDialog.show();
    try {
      FormData from = FormData.fromMap({"userid": "77", "token": "123456789"});
